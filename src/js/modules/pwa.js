...
const renderLogoToCanvas = async (logo, themeHex, storeName, size, isMaskable = false) => {
  ...
  if (isImageUrl) {
    try {
      ...
      if (isMaskable) {
        // Android maskable icon: theme background + safe zone
        ...
        // Favicon & Regular Icons: Gambar logo asli penuh & jernih (tanpa background hijau kotak)
        ctx.clearRect(0, 0, size, size);
        let dw = size, dh = size;
        if (img.width > img.height) dh = size * (img.height / img.width);
        else if (img.height > img.width) dw = size * (img.width / img.height);
        ctx.drawImage(img, (size - dw) / 2, (size - dh) / 2, dw, dh);
      }
  ...
  // Fill background with Theme Color for FontAwesome or Text icons
  ctx.fillStyle = themeHex;
  ctx.fillRect(0, 0, size, size);
