    ...
    pillsBar.innerHTML = categories.map(cat => {
      ...
      return `
        <button type="button" onclick="selectPosCategory('${esc(cat.name)}')" 
          ...
          <span class="text-[10px] px-1.5 py-0.2 rounded-full font-black ml-0.5" style="background-color: ${isSelected ? '#e2e8f0' : 'rgba(0,0,0,0.25)'} !important; color: ${isSelected ? '#0f172a' : '#ffffff'} !important;">${cat.count}</span>
        </button>
      `;
    }).join('');
  }
...
const renderPosProducts = () => {
  ...
  if (posViewMode === 'grid') {
    container.className = 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-3.5 items-stretch';
...
const _getPosMainPrice = (p) => {
  if (p.variants && p.variants.length > 0) {
    const prices = p.variants.map(v => parseFloat(v.price) || 0).filter(pr => pr > 0);
...
const renderPosProductCardGrid = (p) => {
  ...
  return `
    <div class="bg-white dark:bg-slate-900 rounded-2xl border ${isOutOfStock ? 'border-slate-200 dark:border-slate-800 opacity-60' : 'border-slate-200/90 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 hover:shadow-lg'} overflow-hidden transition-all duration-200 cursor-pointer group flex flex-col justify-between h-full active:scale-[0.98] shadow-xs" onclick="${isOutOfStock ? '' : (hasVariants ? `openPosVariantModal('${esc(p.id)}')` : `addToCartPos('${esc(p.id)}', null, 1)`)}" title="${esc(p.name)}">
      
      <!-- Image Container -->
      <div class="relative aspect-square bg-slate-50 dark:bg-slate-800/50 overflow-hidden m-2 sm:m-2.5 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-center shrink-0">
        ...
        ${cartQty > 0 ? `
          <div class="absolute top-1.5 right-1.5 bg-emerald-600 text-white text-[10px] font-black px-2 py-0.5 rounded-lg shadow-md border border-white/40 flex items-center gap-1">
            <i class="fa-solid fa-check text-[9px]"></i>
            <span>${cartQty}</span>
          </div>` : ''}

        <!-- Wholesale Badge (Top Left) -->
        ${isGrosir ? `<div class="absolute top-1.5 left-1.5 bg-amber-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-md shadow-xs uppercase tracking-wider">Grosir</div>` : ''}

        <!-- Variant Count Badge (Bottom Right) -->
        ${hasVariants ? `
          <div class="absolute bottom-1.5 right-1.5 bg-slate-900/80 text-white text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded-md border border-white/20 backdrop-blur-xs flex items-center gap-1">
            <i class="fa-solid fa-layer-group text-[8px]"></i>
            <span>${p.variants.length} Varian</span>
          </div>` : ''}

        <!-- Out of Stock Overlay -->
        ${isOutOfStock ? `<div class="absolute inset-0 bg-white/75 dark:bg-black/75 backdrop-blur-xs flex items-center justify-center"><span class="text-[10px] font-black text-rose-600 bg-white dark:bg-slate-900 px-2.5 py-1 rounded-full shadow-md">Habis</span></div>` : ''}
      </div>

      <!-- Info & Price Content -->
      <div class="px-2.5 pb-2.5 sm:px-3 sm:pb-3 flex flex-col flex-1 justify-between min-w-0">
        <div>
          <h4 class="text-xs sm:text-[13px] font-bold text-slate-800 dark:text-slate-100 leading-snug line-clamp-2 min-h-[2.35rem] max-h-[2.35rem] group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">${esc(p.name)}</h4>
          <p class="text-[9px] sm:text-[10px] font-medium text-slate-400 dark:text-slate-500 truncate mt-0.5 h-4 leading-4">${esc(p.category || 'Umum')}${p.sku ? ' • ' + esc(p.sku) : ''}</p>
        </div>

        <div class="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800 flex items-end justify-between gap-1">
          <div class="min-w-0 flex-1">
            <div class="h-4 leading-4 flex items-center">
              ${isUnlimited ? `<span class="text-[9px] font-bold text-slate-400 leading-none">∞ Stok Bebas</span>` : (hasVariants ? `<span class="text-[9px] font-medium text-slate-400 leading-none">Stok ${totalStock}</span>` : `<span class="text-[9px] font-semibold ${stock > 5 ? 'text-slate-400' : (stock > 0 ? 'text-amber-500 font-bold' : 'text-rose-500 font-bold')} leading-none">Stok ${stock} ${esc(p.unit || 'pcs')}</span>`)}
            </div>
            <div class="text-xs sm:text-[15px] font-black text-emerald-600 dark:text-emerald-400 leading-tight mt-0.5 truncate">${fCur(price)}</div>
          </div>
          
          <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 group-hover:bg-emerald-600 group-hover:text-white flex items-center justify-center text-xs font-bold transition-all shadow-xs shrink-0">
