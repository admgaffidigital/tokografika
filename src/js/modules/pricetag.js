...
window.openPricetagModal = (presetProductIds = null) => {
  ...
  window.openModalSmooth('pricetag-modal', 'pricetag-modal-box', 'bottom-sheet');
};

window.closePricetagModal = () => {
  window.closeModalSmooth('pricetag-modal', 'pricetag-modal-box', 'bottom-sheet');
};
...
const _buildSingleLabelHtml = (item, settings, isThermal = false, cardWidth = 'auto', cardHeight = 'auto') => {
  ...
  let nameFontSize, nameLineH;
  if (nameLen <= 20)       { nameFontSize = '8.5pt'; nameLineH = '1.2'; }
  else if (nameLen <= 32)  { nameFontSize = '7.5pt'; nameLineH = '1.2'; }
  else if (nameLen <= 46)  { nameFontSize = '6.5pt'; nameLineH = '1.2'; }
  else if (nameLen <= 62)  { nameFontSize = '5.5pt'; nameLineH = '1.2'; }
  else                     { nameFontSize = '5pt';   nameLineH = '1.15'; }

  // Clamp maks 3 baris agar tidak mendesak area barcode
  const nameClamp = `display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden;`;
  ...
  if (settings.layoutTemplate === 'gondola_standard') {
    return `
      <div class="pricetag-label-card" style="width:${cardWidth}; height:${cardHeight}; box-sizing:border-box; ${borderStyle} padding:2mm 2.5mm; background:#ffffff; color:#000000; display:flex; flex-direction:column; overflow:hidden; font-family:'Plus Jakarta Sans',Arial,sans-serif; page-break-inside:avoid; border-radius:2px;">

        <!-- HEADER: Nama Toko + Tanggal -->
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #000000; padding-bottom:1.5px; margin-bottom:2px; flex-shrink:0;">
          ${settings.showStoreName ? `<span style="font-size:6pt; font-weight:900; color:#000000; text-transform:uppercase; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:62%;">${esc(storeName)}</span>` : `<span></span>`}
          ${settings.showDate ? `<span style="font-size:5.5pt; font-weight:700; color:#666666; white-space:nowrap; flex-shrink:0;">${dateStr}</span>` : ''}
        </div>

        <!-- NAMA PRODUK: Tidak flex:1 agar tidak buat gap kosong -->
        <div style="overflow:hidden; margin-bottom:2px; flex-shrink:0;">
          <div style="font-size:${nameFontSize}; font-weight:900; color:#000000; line-height:${nameLineH}; text-transform:uppercase; word-break:break-word; ${nameClamp}">
            ${esc(fullName)}
          </div>
        </div>

        <!-- BARCODE + HARGA: margin-top:auto mendorong ke bawah tanpa gap buatan -->
        <div style="display:flex; align-items:flex-end; justify-content:space-between; gap:1mm; margin-top:auto; flex-shrink:0;">
          <!-- Barcode -->
          <div style="flex:1; min-width:0; overflow:hidden;">
            ${settings.showBarcode ? _generatePricetagBarcodeSvg(item.sku, 68, 13) : `<span style="font-size:5.5pt; font-family:monospace; font-weight:bold; word-break:break-all;">${esc(item.sku)}</span>`}
          </div>
          <!-- Harga -->
          <div style="display:flex; flex-direction:column; align-items:flex-end; flex-shrink:0; text-align:right;">
            <div style="font-size:5pt; font-weight:800; color:#555555; line-height:1; white-space:nowrap;">
              ${settings.showUnit ? `PER ${esc(item.unit)}` : 'HARGA'}
            </div>
            <div style="font-size:11pt; font-weight:900; color:#000000; line-height:1.05; letter-spacing:-0.5px; font-family:Arial,sans-serif; white-space:nowrap;">
              ${fCur(item.price)}
            </div>
            ${hasWholesale ? `
              <div style="font-size:4.5pt; font-weight:800; background:#000000; color:#ffffff; padding:0.5px 2px; border-radius:1.5px; margin-top:1px; line-height:1.2; white-space:nowrap;">
  ...
  // ==========================================
    ...
    return `
      <div class="pricetag-label-card" style="width:${cardWidth}; height:${cardHeight}; box-sizing:border-box; ${borderStyle} padding:2mm 2.5mm; background:#ffffff; color:#000000; display:flex; flex-direction:column; overflow:hidden; font-family:'Plus Jakarta Sans',Arial,sans-serif; page-break-inside:avoid; border-radius:2px;">
        ...
        <!-- Header -->
        <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1.5px solid #000000; padding-bottom:1.5px; margin-bottom:2px; flex-shrink:0;">
          <span style="font-size:6pt; font-weight:900; color:#000; text-transform:uppercase; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:58%;">${esc(storeName)}</span>
          <span style="font-size:5pt; font-weight:800; background:#000; color:#fff; padding:1px 2.5px; border-radius:2px; white-space:nowrap; flex-shrink:0; letter-spacing:0.2px;">HEMAT GROSIR</span>
        </div>

        <!-- Nama Produk -->
        <div style="overflow:hidden; margin-bottom:2px; flex-shrink:0;">
          <div style="font-size:${nameFontSize}; font-weight:900; color:#000; line-height:${nameLineH}; text-transform:uppercase; word-break:break-word; ${nameClamp}">
            ${esc(fullName)}
          </div>
        </div>

        <!-- Kotak Harga Eceran + Grosir -->
        <div style="display:flex; justify-content:space-between; align-items:stretch; background:#f8fafc; border:1px solid #000; padding:1.5px 2.5px; border-radius:2px; margin-top:auto; margin-bottom:2px; flex-shrink:0;">
          <div style="display:flex; flex-direction:column; justify-content:center;">
            <span style="font-size:4.5pt; font-weight:bold; color:#444; line-height:1;">ECERAN:</span>
            <span style="font-size:9pt; font-weight:900; color:#000; line-height:1; white-space:nowrap;">${fCur(item.price)}</span>
          </div>
          ${hasWholesale ? `
            <div style="border-left:1px dashed #000; padding-left:3px; text-align:right; display:flex; flex-direction:column; justify-content:center;">
              <span style="font-size:4.5pt; font-weight:900; color:#000; line-height:1;">GROSIR ≥${item.wholesale[0].minQty} ${esc(item.unit)}:</span>
              <span style="font-size:9pt; font-weight:900; color:#000; line-height:1; white-space:nowrap;">${fCur(item.wholesale[0].price)}</span>
            </div>
          ` : `
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-shrink:0;">
          ...
          ${settings.showDate ? `<span style="font-size:5pt; font-weight:bold; color:#666; white-space:nowrap;">${dateStr}</span>` : ''}
        </div>
  ...
  // ==========================================
    ...
    const miniFontSize = miniNameLen <= 18 ? '6.5pt' : miniNameLen <= 28 ? '5.5pt' : '5pt';
    const miniClamp = `display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;`;
    return `
      <div class="pricetag-label-card" style="width:${cardWidth}; height:${cardHeight}; box-sizing:border-box; ${borderStyle} padding:1.5mm 2mm; background:#ffffff; color:#000000; display:flex; flex-direction:column; overflow:hidden; font-family:'Plus Jakarta Sans',Arial,sans-serif; page-break-inside:avoid; text-align:center; border-radius:2px;">

        <!-- Nama Produk (2 baris maks) -->
        <div style="overflow:hidden; margin-bottom:1px; flex-shrink:0;">
        ...
        <!-- Barcode (Center, didorong ke tengah-bawah) -->
        <div style="display:flex; justify-content:center; align-items:center; flex-shrink:0; margin:auto 0 1px;">
          ${settings.showBarcode ? _generatePricetagBarcodeSvg(item.sku, 65, 11) : `<span style="font-size:5pt; font-family:monospace; font-weight:bold;">${esc(item.sku)}</span>`}
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:baseline; border-top:1px solid #000; padding-top:1px; flex-shrink:0;">
          <span style="font-size:5pt; font-weight:bold; color:#444; white-space:nowrap;">${esc(item.unit)}</span>
          <span style="font-size:8.5pt; font-weight:900; color:#000; white-space:nowrap;">${fCur(item.price)}</span>
        </div>
