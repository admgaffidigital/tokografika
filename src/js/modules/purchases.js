    ...
    <div class="space-y-4 sm:space-y-5">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm space-y-3">
          <div class="flex items-center justify-between">
            <h4 class="font-extrabold text-xs sm:text-sm text-slate-800 dark:text-white uppercase tracking-wider flex items-center gap-2">
              <i class="fa-solid fa-money-bill-transfer text-emerald-500"></i> Riwayat Kas Keluar (${allPayments.length})
            </h4>
            <span class="text-[10px] font-bold text-emerald-600">${fCur(totalBayar)} Total Kas</span>
          </div>

          <div class="space-y-2 max-h-[320px] overflow-y-auto pr-1">
            ${!allPayments.length ? `
              <div class="p-6 text-center text-slate-400 text-xs font-medium bg-slate-50 dark:bg-slate-900/60 rounded-xl">
                Belum ada transaksi kas keluar / pembayaran faktur
              </div>
            ` : allPayments.map(pay => {
              return `
                <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800 flex items-center justify-between gap-2.5 text-xs">
                  <div class="min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="font-bold text-slate-800 dark:text-white">${esc(pay.supplierName || 'Supplier')}</span>
                      <span class="font-mono text-[10px] text-slate-400 bg-white dark:bg-slate-800 px-1.5 py-0.2 rounded border border-slate-200 dark:border-slate-700">${esc(pay.invoiceNo)}</span>
                    </div>
                    <p class="text-[10px] text-slate-400 mt-0.5">
                      ${pay.date || '-'} • <span class="capitalize font-semibold text-slate-600 dark:text-slate-300">${esc(pay.note || pay.method || 'Kas Keluar')}</span>
                    </p>
                  </div>
        ...
        <div class="p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm space-y-3">
          <div class="flex items-center justify-between">
            <h4 class="font-extrabold text-xs sm:text-sm text-slate-800 dark:text-white uppercase tracking-wider flex items-center gap-2">
              <i class="fa-solid fa-boxes-stacked text-amber-500"></i> Top Pengadaan Barang
            </h4>
            <span class="text-[10px] font-bold text-slate-400">Berdasarkan Nilai Belanja</span>
          </div>
              ...
              return `
                <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800 flex items-center justify-between gap-2.5 text-xs">
                  <div class="flex items-center gap-2.5 min-w-0">
                    <span class="w-6 h-6 rounded-lg bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 font-black text-[10px] flex items-center justify-center shrink-0">#${idx + 1}</span>
                    <div class="min-w-0">
                      <p class="font-bold text-slate-800 dark:text-white truncate">${esc(tp.name)}${tp.variant ? ` <span class="text-indigo-600 font-semibold text-[10px]">(${esc(tp.variant)})</span>` : ''}</p>
                      <p class="text-[10px] text-slate-400 mt-0.5">
                        Total Masuk: <b class="text-slate-700 dark:text-slate-200">${tp.totalQty} unit</b> • Rata-rata HPP: <b>${fCur(avgCost)}</b>
                      </p>
                    </div>
                  </div>
                  <div class="text-right shrink-0">
