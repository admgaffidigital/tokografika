...
window.rAdmPurchases = () => {
  ...
  const tabBtnClass = (t) => purchasesActiveSubTab === t
    ...
    : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60 font-bold border border-slate-200/80 dark:border-slate-700';
    ...
    <div class="space-y-5 pb-20 w-full">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
        ...
        <div class="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">Total Pembelian Barang</p>
            <h4 class="text-base sm:text-lg font-black text-slate-800 dark:text-white leading-tight mt-0.5">${fCur(totalPurchaseVal)}</h4>
        ...
        <div class="p-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm flex items-center gap-3.5">
          ...
          <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">Total Sudah Terbayar</p>
            <h4 class="text-base sm:text-lg font-black text-indigo-600 dark:text-indigo-400 leading-tight mt-0.5">${fCur(totalPaidVal)}</h4>
        ...
        <div class="p-4 rounded-2xl bg-white dark:bg-slate-800 border ${totalRemainingDebt > 0 ? 'border-amber-300 dark:border-amber-700/80 bg-amber-50/20' : 'border-slate-200/80 dark:border-slate-700'} shadow-sm flex items-center gap-3.5">
          <div class="w-12 h-12 rounded-2xl ${totalRemainingDebt > 0 ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'} flex items-center justify-center text-xl shrink-0">
            <i class="fa-solid fa-hourglass-half"></i>
          </div>
          <div>
            <p class="text-[10px] font-bold ${totalRemainingDebt > 0 ? 'text-amber-700 dark:text-amber-300 font-black' : 'text-slate-400'} uppercase tracking-wider leading-tight">Total Sisa Hutang Supplier</p>
            <h4 class="text-base sm:text-lg font-black text-amber-600 dark:text-amber-400 leading-tight mt-0.5">${fCur(totalRemainingDebt)}</h4>
      ...
      <div class="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar pb-1">
        <div class="flex items-center gap-2 shrink-0">
          <button type="button" class="px-4 py-2 rounded-xl text-xs transition-all flex items-center gap-1.5 ${tabBtnClass('invoices')}" onclick="setPurchasesSubTab('invoices')">
            <i class="fa-solid fa-file-invoice text-xs"></i>
            <span>Faktur Pembelian (${purchases.length})</span>
          </button>

          <button type="button" class="px-4 py-2 rounded-xl text-xs transition-all flex items-center gap-1.5 ${tabBtnClass('debts')}" onclick="setPurchasesSubTab('debts')">
          ...
          <button type="button" class="px-4 py-2 rounded-xl text-xs transition-all flex items-center gap-1.5 ${tabBtnClass('suppliers')}" onclick="setPurchasesSubTab('suppliers')">
            <i class="fa-solid fa-truck-field text-xs"></i>
            <span>Data Supplier (${suppliers.length})</span>
          </button>

          <button type="button" class="px-4 py-2 rounded-xl text-xs transition-all flex items-center gap-1.5 ${tabBtnClass('reports')}" onclick="setPurchasesSubTab('reports')">
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button type="button" class="btn-solid no-glass px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs flex items-center gap-1.5 shadow-md active:scale-95 transition-all" onclick="openPurchaseModal()">
