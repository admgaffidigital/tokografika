...
setH('dynamic-banners-container', `
 ...
 ${(appData.banners || []).map((b, i) => {
    const hasImg = b.img && b.img.trim().length > 0;
    const hasTitle = b.title && b.title.trim().length > 0;
    
    if (hasImg) {
        return `
        <div class="w-[88%] sm:w-[520px] md:w-[640px] aspect-[16/7] sm:aspect-[21/9] snap-center shrink-0 rounded-2xl overflow-hidden shadow-md relative border border-slate-200 dark:border-slate-700 bg-slate-900 group cursor-pointer">
            <img data-src="${esc(b.img)}" src="data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxIDEiPjwvc3ZnPg==" onerror="this.onerror=null;this.src='https://placehold.co/800x350?text=Banner'" class="lazy-load opacity-0 transition-all duration-700 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500"/>
            ${hasTitle ? `
            <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/25 to-transparent flex flex-col justify-end p-4 sm:p-5 text-white pointer-events-none">
    ...
    return `
    <div class="w-[85%] sm:w-[320px] snap-center shrink-0 rounded-2xl p-5 text-white shadow-md relative overflow-hidden flex items-center justify-between border-2 border-transparent" style="background: ${i % 2 === 0 ? 'var(--clr-p)' : 'var(--clr-p-dark)'};">
...
window.showPwaBanner = () => {
 ...
 if (icon) icon.className = 'fa-solid fa-arrow-up- text-white text-xl';
